...
uniform vec3 TopLeftSpawnBound;
uniform vec3 BottomRightSpawnBound;
...
vec3 randomInitialVelocity(){
    ...
    return normalize(vec3(0, 1, 0) * v) * velocity;
}

//vec3 randomInitialPosition(){
    //float x = mix(TopLeftSpawnBound.x, BottomRightSpawnBound.x, texelFetch(RandomTex, 3 * gl_VertexID, 0).r);
   // float y = mix(TopLeftSpawnBound.y, BottomRightSpawnBound.y, texelFetch(RandomTex, 3 * gl_VertexID + 1, 0).r);
   // float z = mix(TopLeftSpawnBound.z, BottomRightSpawnBound.z, texelFetch(RandomTex, 3 * gl_VertexID + 2, 0).r);
    //return vec3(x, y, z);
//}

vec3 randomInitialPosition() {
    float x = texelFetch(RandomTex, (3 * gl_VertexID + 0) % 300, 0).r;
    float y = texelFetch(RandomTex, (3 * gl_VertexID + 1) % 300, 0).r;
    float z = texelFetch(RandomTex, (3 * gl_VertexID + 2) % 300, 0).r;
    return vec3(x * 4.0f, y * 4.0f, z * 4.0f);
}

vec3 updateParticleVel(){
    float vx = texelFetch(RandomTex, 3 * gl_VertexID, 0).r;
    float vy = texelFetch(RandomTex, 3 * gl_VertexID + 1, 0).r;
    float vz = texelFetch(RandomTex, 3 * gl_VertexID + 2, 0).r;
    vec3 randomVel = vec3(vx, vy, vz);

    vec3 newVel = mix(VertexVelocity, randomVel, 0.1);
    return newVel;
}
